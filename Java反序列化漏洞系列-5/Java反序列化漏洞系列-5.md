...
|     |     |     |
| --- | --- | --- |
| ```bash<br>1<br>2<br>3<br>4<br>5<br>6<br>7<br>8<br>``` | ```java<br>public static void bruteHashCode()<br>{<br>    for (long i = 0; i < 9999999999L; i++) {<br>        if (Long.toHexString(i).hashCode() == 0) {<br>            System.out.println(Long.toHexString(i));<br>        }<br>    }<br>}<br>``` |
